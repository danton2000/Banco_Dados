...
ANA.SALARIO,
CASE
	WHEN COUNT(ATI.DESCRICAO) = 1 THEN CAST(ANA.SALARIO * 1.05 AS NUMERIC(15,2))
...
SELECT 
ANA.CODANALISTA, ANA.ANALISTA, CUR.CURSO, ATIANA.codatividadeanalise
FROM ANALISTA ANA
INNER JOIN CURSO CUR ON CUR.CODCURSO = ANA.CODCURSO
INNER JOIN ATIVIDADESANALISE ATIANA ON ATIANA.CODANALISTA = ANA.CODANALISTA
WHERE NOT EXISTS (
	SELECT ATIVPRO.CODATIVIDADEANALISE
	FROM PROGRAMADOR PRO
	INNER JOIN ATIVIDADESPROG ATIVPRO ON ATIVPRO.CODPROGRAMADOR = PRO.CODPROGRAMADOR
	WHERE PRO.PROGRANADOR LIKE '%Jefer%'
)
